...
export default function Input({
  ...
  function generateText() {
    ...
    async function getExtracts(input) {
        const url = `https://es.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(input)}`;
        
        try {
          const response = await fetch(url, {
            headers: { 'User-Agent': 'mc-txt/1.0' } // Required by Wikipedia
          });
          const data = await response.json();
          
          return data.extract || "No se encontró información.";
        } catch (err) {
          return "";
        }
      }
  ...
  return (
    <div id="content" className="content">
      <h1 className="title" onClick={() => setCurrentStep(0)}>
        mc-txt
      </h1>
      ...
      <div
        id="contenedor-input"
        className="div-container nes-container with-title"
      >
        ...
        <input
          ...
          placeholder="un deseo"
        ></input>
        <input
          id="fear"
          defaultValue={currentFear}
          onChange={getFear}
          type="text"
          className="input nes-input"
          placeholder="un miedo"
        ></input>
        <input
          id="smell"
          defaultValue={currentSmell}
          onChange={getSmell}
          type="text"
          className="input nes-input"
          placeholder="un olor"
        ...
        <input
          ...
          type="text"
          className="input nes-input"
          placeholder="un color"
        ></input>
        <input
          id="passion"
          defaultValue={currentPassion}
          onChange={getPassion}
          type="text"
          className="input nes-input"
          placeholder="una pasión"
        ></input>
      </div>
      <div className="controls">
        <label>
          <input
            ...
            name="text"
          />
          <span className="nes-text" onClick={clearInput}>
            borrar
          </span>
        </label>
        <label>
          <input
            ...
            defaultChecked
          />
          <span className="nes-text" onClick={() => generateText()}>
            continuar
          </span>
        </label>
      </div>
